...
app.post('/api/auth/register/send-otp', authLimiter, async (req, res) => {
  ...
  if (await User.findOne({ userId: key }))
...
app.post('/api/auth/register/verify-otp', authLimiter, async (req, res) => {
  ...
  if (await User.findOne({ userId: key }))
    return res.status(400).json({ error: 'This User ID was just taken. Please choose another.' });
...
app.post('/api/auth/forgot/send-otp', forgotLimiter, async (req, res) => {
  ...
  if (user.method==='email'||user.email) sendEmailOTP(contact, otp, 'reset').catch(e=>console.error('Forgot OTP email error:', e.message));
  else sendPhoneOTP(contact, otp).catch(e=>console.error('Forgot OTP phone error:', e.message));
});
...
app.post('/api/auth/settings/update-userid', auth, async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9_]{3,20}$/.test(newUserId))
    return res.status(400).json({ error: 'User ID: 3-20 chars, letters/numbers/underscore only.' });
  if (isGlobalAdmin(newUserId)) return res.status(400).json({ error: 'This User ID is reserved.' });
  const user = await User.findOne({ userId: req.session.user.userId });
  if (!user) return res.status(404).json({ error: 'User not found.' });
  if (!(await bcrypt.compare(currentPassword, user.password)))
    return res.status(400).json({ error: 'Current password is incorrect.' });
  if (await User.findOne({ userId: newUserId.toLowerCase() }))
    return res.status(400).json({ error: 'This User ID is already taken.' });
  user.userId = newUserId.toLowerCase(); await user.save();
  req.session.user = safe(user);
...
app.post('/api/auth/change-password', auth, async (req, res) => {
  ...
  if (!(await bcrypt.compare(currentPassword, user.password)))
    return res.status(400).json({ error: 'Current password is incorrect.' });
  if (!user.securityAnswer) return res.status(400).json({ error: 'No security question set on this account.' });
...
app.put('/api/admin/users/:uid/role', auth, admin, async (req,res) => {
  ...
  if (isGlobalAdmin(targetUid)) return res.status(403).json({ error:'The global admin role cannot be changed.' });
...
app.delete('/api/admin/users/:uid', auth, admin, async (req,res) => {
  const targetUid = req.params.uid.toLowerCase();
  if (isGlobalAdmin(targetUid)) return res.status(403).json({ error:'The global admin account cannot be deleted.' });
  const u = await User.findOne({ userId:targetUid });
  if (!u) return res.status(404).json({ error:'User not found.' });
  if (u.userId===req.session.user.userId) return res.status(400).json({ error:'You cannot delete your own account.' });
  ...
  if (GEMINI_KEYS.length) {
    ...
    for (const model of geminiModels) {
      for (const keyIdx of keyOrder) {
        try {
          const response = await callGemini(model, GEMINI_KEYS[keyIdx]);
